...
def createHTML(name, pathin, pathout):
    print("pathin", pathin)
    print("pathout", pathout)
    img = cv.imread(pathin, cv.COLOR_BGR2HSV)
    print(img.shape)
    w = createWebWidgets(img)

    cp.create_html(name[:-4] + ".html", cp.create_sections(w), pathout)